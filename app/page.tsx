...
const PLACEHOLDER_EXAMPLES = [
  ...
  "Why does helping startups succeed matter to Christian?",
  "Why should Mercury hire Christian?",
];

const SUGGESTED_QUESTIONS = [
  "What are Christian's top three qualifications as a candidate for this role?",
  "Why does helping startups succeed matter to Christian, personally?",
  "How has Christian built pipeline or a sales motion from scratch when there was no playbook to follow?",
  "How does Christian use cross-platform intelligence to win new logos and expand existing accounts?",
  "Why should Mercury hire Christian?",
];
...
function MercuryLogo() {
  return (
    <div className="flex items-center gap-2" aria-label="Mercury logo">
      <span className="block h-4 w-4 rounded-full bg-[#5266EB]" />
...
export default function Home() {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-12 sm:py-20">
      ...
      <h1 className="mb-8 mt-5 text-3xl font-medium tracking-tight text-black sm:text-4xl">
        I&apos;m Christian, and I want to work for Mercury
      </h1>
      ...
      <div className="mt-8 rounded-md border border-neutral-200 bg-white shadow-sm">
            ...
            {messages.map((m, i) =>
              m.role === "user" ? (
                <p
                  ...
                  className={`scroll-mt-4 text-[16px] font-bold text-black ${
                ...
                <div
                  key={i}
                  className="mt-3 space-y-3 text-[16px] leading-[1.75] text-neutral-700 [&_ol]:list-decimal [&_ol]:space-y-1.5 [&_ol]:pl-5 [&_strong]:font-bold [&_strong]:text-black [&_ul]:list-disc [&_ul]:space-y-1.5 [&_ul]:pl-5"
                >
        ...
        >
          <input
            ...
            disabled={loading}
            className="flex-1 bg-transparent text-[16px] text-black placeholder:text-neutral-400 focus:outline-none disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={loading || !input.trim()}
            className="text-[15px] font-bold text-[#5266EB] hover:text-[#3d4fd1] disabled:text-neutral-300"
          >
            Send
          </button>
        </form>
      ...
      <div className="mt-12 flex flex-col">
        {SUGGESTED_QUESTIONS.map((q, i) => (
          <button
            ...
            disabled={loading}
            className={`group flex items-start gap-2 border-t border-neutral-200 px-2 py-2.5 text-left text-[15px] text-neutral-600 transition-colors hover:bg-neutral-50 hover:text-[#5266EB] disabled:opacity-50 ${
              i === SUGGESTED_QUESTIONS.length - 1 ? "border-b" : ""
            }`}
          >
            <span
              aria-hidden
              className="mt-0.5 shrink-0 text-neutral-400 transition-colors group-hover:text-[#5266EB]"
            >
              →
            </span>
            <span>{q}</span>
